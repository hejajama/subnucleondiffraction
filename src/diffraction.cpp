...
double Diffraction::ScatteringAmplitude(double xpom, double Qsqr, double t, double theta_DeltaT, Polarization pol)
{
    ...
    helper.theta_DeltaT = theta_DeltaT;
...
double Inthelperf_amplitude_mc( double *vec, size_t dim, void* par)
{
    ...
    return helper->diffraction->ScatteringAmplitudeIntegrand(helper->xpom, helper->Qsqr, helper->t, helper->r, helper->theta_r, helper->b, helper->theta_b, z, helper->theta_DeltaT, helper->polarization);
...
double Inthelperf_amplitude_z(double z, void* p)
{
    ...
    return helper->diffraction->ScatteringAmplitudeIntegrand(helper->xpom, helper->Qsqr, helper->t, helper->r, helper->theta_r, helper->b, helper->theta_b, z, helper->theta_DeltaT, T);
}

double Diffraction::ScatteringAmplitudeIntegrand(double xpom, double Qsqr, double t, double r, double theta_r, double b, double theta_b, double z, double theta_DeltaT, Polarization pol)
{ 
    ...
    else
    {
        ...
        std::complex<double> exponent = std::exp( -imag* ( b*delta*std::cos(theta_b - theta_DeltaT) - (0.5 - z)*r*delta*std::cos(theta_r - theta_DeltaT)  )  );
